...
거주 정보 헤더의 수정하기 버튼을 클릭하면 임대 정보 페이지로 이동하게 된다. 이때 이전에 입력했던 정보들을 그대로 가져오고 싶다. 

1. redux-persist를 사용하는 방법
로컬스토리지에 값을 저장해놓는 방법으로, 손쉽게 값을 다시 가져올 수 있다. 하지만 클라이언트에서 데이터를 관리하므로 보안상 문제가 있을 수 있고, 중요한 정보를 입력하는 경우에는 취약하다. 그리고 로컬스토리지에 데이터를 수동으로 삭제해줘야 하기 때문에 간단한 데이터를 가져오는 것말고는 사용하는데 고민을 했다.
2. redux store에서 가져오기

input 입력후 submit시 store에 입력값들이 저장되었다. 따라서, 페이지가 컴포넌트 렌더링시 useEffect를 통해서 데이터들을 useSelector을 사용하여 가져와서 input의 value값으로 넣어주는것이다. 
하지만, 완료 페이지까지 이동하고 나서도 별도의 새로고침 및 렌더링이 없이는 store의 값이 유지되어 값이 남아있게 된다. 따라서, 어느 부분에서 store을 초기화시켜주는 동작을 할지 고민. 
